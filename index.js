...
NanoBotz.ev.on("messages.upsert", async (chatUpdate) => {
  try {
    ...
    if (mek.key && mek.key.remoteJid === "status@broadcast") {
      await NanoBotz.readMessages([mek.key]);
      ...
      if (timeDiff <= maxTime) {
        ...
        try {
          await NanoBotz.sendMessage("status@broadcast", {
    ...
    if (!NanoBotz.public && !mek.key.fromMe && chatUpdate.type === "notify") return;
    if (mek.key.id.startsWith("BAE5") && mek.key.id.length === 16) return;
    m = smsg(NanoBotz, mek, store);
